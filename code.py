...
def shear_rotate(bmp, angle, palette, debug=False):
    clockwise = angle > 0
    ...
    #     h_multiplier = -math.tan(angle / 2)
    #     v_multiplier = math.sin(angle)
    #     print(f"Inside h_multiplier: {h_multiplier}, v_multiplier: {v_multiplier}")
    ...
    first_row_shear_amount = abs(round(first_row_centered_index * h_multiplier))
    ...
    if debug:
        shear_rotated_tg = TileGrid(output_bmp, pixel_shader=palette, width=1, height=1,
                                    tile_width=output_bmp.width, tile_height=output_bmp.height)
    ...
    # vertical shear
    for col_index in range(output_bmp.width - first_row_shear_amount):
        centered_col_index = output_bmp.width // 2 - col_index
        ...
        if True:  # col_index == 0:
            ...
            # bitmaptools.fill_region(output_bmp, x1, y1, x2, y2, 2)
            # print(f"draw dot x:{x} y:{y} x1:{x+1} y1:{y+1}")
            # bitmaptools.fill_region(output_bmp, x, y, x + 1, y + 1, 1)
...
sprite_sheet, palette = adafruit_imageload.load("/colorbox_16x16.bmp",
                                                bitmap=Bitmap,
                                                palette=Palette)
...
rotated_bmp = Bitmap(24, 24, len(palette))

bitmaptools.rotozoom(dest_bitmap=rotated_bmp, source_bitmap=sprite_sheet,
                     # source_clip0=(0,0), source_clip1=(16,16), px=rotated_bmp.width//2, py=rotated_bmp.height//2, angle=0.785398)
                     source_clip0=(0, 0), source_clip1=(16, 16), px=sprite_sheet.width // 2, py=sprite_sheet.width // 2,
                     angle=0.785398)
...
shear_rotated_bmp = shear_rotate(sprite_sheet, 0.785398, palette)
...
rotated_back_bmp = shear_rotate(shear_rotated_bmp, -0.785398, palette)
rotated_back_tg = TileGrid(rotated_back_bmp, pixel_shader=palette, width=1, height=1,
                           tile_width=rotated_back_bmp.width, tile_height=rotated_back_bmp.height)

rotated_back_tg.y = 20
rotated_back_tg.x = 4
main_group.append(rotated_back_tg)
